...
public struct TensorImage {
    ...
    public func save(to url: URL, quality: Int64 = 95) {
        ...
        let encodedJpeg = Raw.encodeJpeg(
            image: outputImageData, format: .rgb, quality: quality, xmpMetadata: "")
        Raw.writeFile(filename: StringTensor(url.absoluteString), contents: encodedJpeg)
    }

    public func resized(to size: (Int, Int)) -> TensorImage {
        switch self.imageData {
        case let .uint8(data):
            return TensorImage(
...
public func saveImage(_ tensor: Tensor<Float>, size: (Int, Int, Int), directory: String, name: String) throws {
    try createDirectoryIfMissing(at: directory)
    let reshapedTensor = tensor.reshaped(to: [size.0, size.1, size.2])
    let image = TensorImage(tensor: reshapedTensor)
